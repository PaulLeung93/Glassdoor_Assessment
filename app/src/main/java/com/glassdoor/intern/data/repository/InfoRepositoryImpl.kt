        ...
        try {
            with(infoApi.getInfo()) {
                when {
                    ...
                    error != null -> Err(Throwable("Error: $error"))//DONE: ("Convert to error")
                    else -> Err(Throwable("Error with getHeaderInfo()"))//DONE: ("Convert to error")
                }
            }
        } catch (throwable: Throwable) {
            Timber.e(throwable, "InfoRepositoryImpl")

            Err(throwable)  //DONE: ("Convert to error")
        }
}
